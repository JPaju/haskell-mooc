...
distance x1 y1 x2 y2 = sqrt (deltaX + deltaY)
  where
    deltaX = (x2 - x1) ^ 2
    deltaY = (y2 - y1) ^ 2
...
checkPassword password =
  let allowedPasswords = Set.fromList ["swordfish", "mellon"]
   in if Set.member password allowedPasswords
        then "You're in."
        else "ACCESS DENIED!"
