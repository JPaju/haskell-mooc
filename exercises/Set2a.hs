...
updateAt :: Int -> a -> [a] -> [a]
updateAt _ _ [] = []
updateAt 0 a (x : xs) = a : xs
updateAt i a (x : xs) = x : updateAt (i -1) a xs
...
palindromify s
  | isPalindrome s = s
  | otherwise = palindromify chopped
  where
    chopped = substring 1 lastIdx s
    lastIdx = length s - 1
...
greet first maybeLast = "Hello, " ++ name ++ "!"
  where
    name = maybe first (\last -> first ++ " " ++ last) maybeLast
...
eitherDiv x y = toEither errMsg $ safeDiv x y
  where
    toEither _ (Just b) = Right b
    toEither a Nothing = Left a
    errMsg = show x ++ "/" ++ show y
...
addEithers (Left str) _ = Left str
addEithers _ (Left str) = Left str
