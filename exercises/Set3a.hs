...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if ma > mb then a else b
  where
    ma = measure a
    mb = measure b
...
firstHalf str = take halfPoint str
  where
    len = length str
    halfLen = len `div` 2
    halfPoint = if even len then halfLen else 1 + halfLen
...
-- capitalize str = str |> words |> map capitalizeWord |> unwords
...
joinToLength n strs = filter (length >>> (== n)) concatenations
  where
    concatenations = [a ++ b | a <- strs, b <- strs, a /= b]
...
interpreter commands = helper 0 0 commands []
  where
    helper :: Int -> Int -> [String] -> [String] -> [String]
    helper x y ("up" : cmds) acc = helper x (y + 1) cmds acc
    helper x y ("down" : cmds) acc = helper x (y - 1) cmds acc
    helper x y ("left" : cmds) acc = helper (x - 1) y cmds acc
    helper x y ("right" : cmds) acc = helper (x + 1) y cmds acc
    helper x y ("printX" : cmds) acc = helper x y cmds (acc ++ [show x])
    helper x y ("printY" : cmds) acc = helper x y cmds (acc ++ [show y])
    helper x y (_ : cmds) acc = helper x y cmds acc
    helper x y [] acc = acc
